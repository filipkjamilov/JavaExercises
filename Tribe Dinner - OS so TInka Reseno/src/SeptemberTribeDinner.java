...
public class SeptemberTribeDinner {
    ...
    static Semaphore dali_kazanot_e_prazen;
    static Semaphore kuvar_gotvi;
    static Semaphore cekam_na_kuvarot;
    static Semaphore jadat_na_trpeza;

    public static void init() {

        dali_kazanot_e_prazen = new Semaphore(1);
        kuvar_gotvi = new Semaphore(0);
        cekam_na_kuvarot = new Semaphore(0);
        jadat_na_trpeza = new Semaphore(4);
    }
    ...
    public static class TribeMember extends TemplateThread {
        ...
        public void execute() throws InterruptedException {
            ...
            dali_kazanot_e_prazen.acquire();
            if(state.isPotEmpty()){
                //Znaci deka kazanot e prazen
                //Treba da se povika sefot da zgotvi
                kuvar_gotvi.release();
                //Baram pristap da pominam, ke dobijam otkako sefot ke zgotvi
                cekam_na_kuvarot.acquire();
                state.fillPlate();
            }else{
                state.fillPlate();
            }
            dali_kazanot_e_prazen.release();

            jadat_na_trpeza.acquire();
            state.eat();
            jadat_na_trpeza.release();
    ...
    public static class Chef extends TemplateThread {
        ...
        public void execute() throws InterruptedException {
            ...
            kuvar_gotvi.acquire();
            //Cekame permisija od nekoj od clenovite da dade relese na semaforot
            //Potoa kuvarot moze da zgotvi hrana vo kazanot
            state.cook();
            //Go osloboduvam clenot da jade
            cekam_na_kuvarot.release();
