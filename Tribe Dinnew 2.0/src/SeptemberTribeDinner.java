...
public class SeptemberTribeDinner {
    ...
    static Semaphore kuvar;
    static Semaphore jadam;

    static Semaphore zgotveno;


    public static void init() {

        proveri = new Semaphore(1);
        jadam = new Semaphore(4);
        kuvar = new Semaphore(0);
        zgotveno = new Semaphore(0);
    ...
    public static class TribeMember extends TemplateThread {
        ...
        public void execute() throws InterruptedException {
            ...
            //Vleguva 1 klient za da zeme hrana
            proveri.acquire();
            if (!state.isPotEmpty()) {
                // 1klient polni hrana
                state.fillPlate();
                // se osloboduva eden klient
                proveri.release();
                //Moze da vlezat vo salata za jadenje max 4
                jadam.acquire();
                state.eat();
                jadam.release();
                //Od salata koj kako ke se najade taka si odi i vleguvaat drugi klienti

            } else {
                //Tuka nemame hrana i go vikame kuvarot
                kuvar.release();
                //kuvarot veke zgotvil
                zgotveno.acquire();
                //klientot zema hrana samo eedeenn!!!
                state.fillPlate();
                proveri.release();
                //go osloboduvame klientott!
                jadam.acquire();
                //Vo salata moze da vlezat 4 max
                state.eat();
                jadam.release();
                //Od salata koj kako ke se najade taka si odi i vleguvaat drugi klienti
            }
    ...
    public static class Chef extends TemplateThread {
        ...
        @Override
        public void execute() throws InterruptedException {
            // Kuvarot ceka da se povika za da gotvvi
            kuvar.acquire();
            state.cook();
            //koga kuvarot zavrsuva kazuva na klientite deka zavrsil
            zgotveno.release();
