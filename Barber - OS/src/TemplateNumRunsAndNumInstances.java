...
public class TemplateNumRunsAndNumInstances {
    ...
    static Semaphore entry;
    static Semaphore sits;
    static Semaphore call;
    static boolean flag;
    static int clients;
    ...
    public static void init(int numBarbers) {
        ...
        sits = new Semaphore(0);
        call = new Semaphore(0);
        clients = 0;
        flag = true;
    }
    ...
    static class Barber extends TemplateThread {
        ...
        public void execute() throws InterruptedException {
            ...
            // koga 5tiot klient ke notificira, berberot treba da se wakeUp
            if (flag) {
                wakeUp.acquire();
                flag = false;
                state.barberWakeUp();
            ...
            state.cutHair();
            // proveruva dali ima clients koi cekaat, ako nema
            // ovde vleguva i ne dozvoluva da vleze nov klient
            entry.acquire();
            if (clients == 0) {
    ...
    static class Consumer extends TemplateThread {
        ...
        public void execute() throws InterruptedException {
            ...
            clients++;
            if (clients == 5&&flag){
                //Koga klientite ke se soberat 5 togash berberot se budi
                wakeUp.release();
            }
            entry.release();
