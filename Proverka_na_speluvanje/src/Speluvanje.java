...
class Zbor implements Comparable<Zbor>{
    ...
    // Tuka treba override na hashCode() ama neznam funkcija
...
public class Speluvanje {
    public static void main(String[] args) throws IOException {
        ...
        for(int i=0; i<N; i++){
            ...
            Zbor pom = new Zbor(line);
            tabela.insert(pom, line);
        }

        String recenica = br.readLine();
        if (recenica.equals(".")){
            System.out.println("Bravo");
            //Bravo za Finki Ukim MK .!.
            return;
        }
        String pom = recenica.replaceAll("\\.", " ");
        pom = pom.replaceAll(",", " ");
        pom = pom.replaceAll("!", " ");
        pom = pom.replaceAll("\\?", " ");
        pom = pom.replaceAll(" ", " ");
        pom = pom.trim().replaceAll(" +", " ");
        String []line = pom.split(" ");

        boolean f2 = false;
        for (int i=0; i<line.length; i++){

            String zbor = line[i].toLowerCase();
            Zbor p = new Zbor(zbor);

            int index = tabela.search(p);
